...
def enrich_sl_entries_with_procurement_details(sl_entries, filters, items):
	...
	if pr_names:
		res = frappe.get_all(
			...
			fields=["name", "posting_date", "currency", "is_return", "return_against"]
		)
	...
	for s in all_sles:
		if s.actual_qty > 0:
			info = {
				"origin_voucher_no": s.voucher_no, "origin_voucher_type": s.voucher_type,
				"origin_posting_date": s.posting_date, "origin_posting_time": s.posting_time,
	...
	for s in all_sles:
		...
		if s.actual_qty > 0:
			...
			return_against_pr = None
			if s.voucher_type == "Purchase Receipt":
				pr_hdr = pr_data.get(s.voucher_no)
				if pr_hdr and pr_hdr.get("is_return") and pr_hdr.get("return_against"):
					return_against_pr = pr_hdr.get("return_against")

			while qty_to_consume > 0 and fifo_queues[key]:
				batch_idx = 0
				if return_against_pr:
					for idx, b in enumerate(fifo_queues[key]):
						if b["procurement"].get("purchase_receipt") == return_against_pr:
							batch_idx = idx
							break
				
				batch = fifo_queues[key][batch_idx]
	...
	for sle in sl_entries:
		if sle.actual_qty < 0:
			sle.total_out_voucher_qty = abs(sle.actual_qty)
		else:
			sle.total_out_voucher_qty = 0
